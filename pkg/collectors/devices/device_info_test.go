...
var _ = Describe("NewContainerContext", func() {
	...
	When("called GetPTPDeviceInfo", func() {
		It("should return a valid PTPDeviceInfo", func() {
			...
			response[`sh -c 'ls /sys/class/net/aFakeInterface/device/gnss/ 2>/dev/null || true'`] = Response{stdout: gnssDev}

			expectedInput := "echo '<date>';date +%s.%N;echo '</date>';"
			expectedInput += "echo '<gnss>';sh -c 'ls /sys/class/net/aFakeInterface/device/gnss/ 2>/dev/null || true';echo '</gnss>';"
